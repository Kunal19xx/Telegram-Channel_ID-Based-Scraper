...
parser.add_argument(
    '--telegram-channel-id',
    type=str,
    required= all(['--batch-file' not in sys.argv,
                   '--telegram-channel' not in sys.argv]),
...
parser.add_argument(
    ...
    type=str,
    required=all(['--batch-file' not in sys.argv,
                  '--telegram-channel-id' not in sys.argv]),
    help='Specifies a Telegram Channel.'
)
parser.add_argument(
    '--batch-file',
    type=str,
    required=all(['--telegram-channel' not in sys.argv,
                 '--telegram-channel-id' not in sys.argv]),
...
phone = args['phone']
channel_id_flag = True if "telegram_channel_id" in args.keys() else False
# event loop
...
for channel in req_input:
    ...
    if channel_id_flag:
        # Use Channel ID | convert output to dict
    ...
    if channel_found_flag:
        ...
        if channel_id_flag:
            w.write(f'ID - {channel}\n')
        else:
            w.write(f'Name - {channel}\n')
        w.close()
